...
#include "res://shaders/includes/sdfs/sdf_mandelbulb_64.gdshaderinc"
#include "res://shaders/includes/color/PBR.gdshaderinc"
#include "res://shaders/includes/rayMarcher/ray_marcher_AR_64.gdshaderinc"

void main() {
    ivec2 base_coord = ivec2(gl_GlobalInvocationID.xy) * resolution_scale;
    ivec2 image_size = imageSize(output_image);

    if (base_coord.x >= image_size.x || base_coord.y >= image_size.y) {
        return;
    }
    ...
    dvec3 rayDir64 = getRayDirection64(cam.resolution, uv);
    vec3 color = raymarch_AR64(rayDir64);
